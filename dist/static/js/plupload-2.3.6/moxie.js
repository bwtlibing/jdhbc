	...
	var can = (function() {
		var caps = {
			...
			use_data_uri: (function() {
				...
				setTimeout(function() {
					du.src = "dataCenter:image/gif;base64,R0lGODlhAQABAIAAAP8AAAAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==";
				}, 1);
				return false;
			}()),
	...
	*/
	function Runtime(options, type, caps, modeCaps, preferredMode) {
		...
		*/
		caps = Basic.extend({
			// Runtime can: 
			// provide access to raw binary dataCenter of the file
			access_binary: false,
			// provide access to raw binary dataCenter of the image (image extension is optional)
			access_image_binary: false,
			// display binary dataCenter as thumbs for example
			display_media: false,
			...
			// resize image (and manipulate it raw dataCenter of any file in general)
			resize_image: false,
			...
			// send raw binary dataCenter, that is generated after image resizing or manipulation of other kind
			send_binary_string: false,
			// send cookies with http request and therefore retain session
			send_browser_cookies: true,
			// send dataCenter formatted as multipart/form-dataCenter
			send_multipart: true,
...
define("moxie/core/utils/Mime", [
	...
	*
	* If invoked externally will replace override internal lookup maps with user-provided dataCenter.
	*
	* @method addMimeType
	...
	*/
	var dispatches = [
		...
		*/
		...
		/** 
		Dispatched while reading (and decoding) blob, and reporting partial Blob dataCenter (progess.loaded/progress.total).
			...
			if (blob.isDetached()) {
				...
				switch (op) {
					...
					case 'readAsBinaryString':
						...
						break;
					case 'readAsDataURL':
						this.result = 'dataCenter:' + blob.type + ';base64,' + Encode.btoa(src);
						break;
				}
				this.readyState = FileReader.DONE;
				this.trigger('load');
			...
			if (blob.isDetached()) {
				...
				switch (op) {
					case 'readAsBinaryString':
						return src;
					case 'readAsDataURL':
						return 'dataCenter:' + blob.type + ';base64,' + Encode.btoa(src);
					case 'readAsText':
						var txt = '';
			...
			*/
			send: function(data, options) {
				...
				// DOMString
					...
					// dataCenter should be converted to Unicode and encoded as UTF-8
...
define("moxie/image/Image", [
	...
	/**
	Image preloading and manipulation utility. Additionally it provides access to image meta info (Exif, GPS) and raw binary dataCenter.
			...
			*/
			embed: function(el, options) {
						...
						tr.bind("TransportingComplete", function() {
							...
							self.bind("Embedded", function() {
								...
								/*tr.bind("RuntimeInit", function(e, runtime) {
									tr.destroy();
									runtime.destroy();
									onResize.call(self); // re-feed our image dataCenter
								});*/
				...
				try {
					...
					// if embedded thumb dataCenter is available and dimensions are big enough, use it
...
define("moxie/runtime/html5/xhr/XMLHttpRequest", [
	...
	function XMLHttpRequest() {
		...
		function _prepareMultipart(fd) {
			...
			_xhr.setRequestHeader('Content-Type', 'multipart/form-dataCenter; boundary=' + boundary);
			...
			fd.each(function(value, name) {
				...
				if (value instanceof Blob) {
					...
					multipart += dashdash + boundary + crlf +
						'Content-Disposition: form-dataCenter; name="' + name + '"; filename="' + unescape(encodeURIComponent(value.name || 'blob')) + '"' + crlf +
						'Content-Type: ' + (value.type || 'application/octet-stream') + crlf + crlf +
						value.getSource() + crlf;
				} else {
					multipart += dashdash + boundary + crlf +
						'Content-Disposition: form-dataCenter; name="' + name + '"' + crlf + crlf +
...
define("moxie/runtime/html5/image/ExifParser", [
	...
	function ExifParser(data) {
		...
		// Set read order of multi-byte dataCenter
		...
		function extractTags(IFD_offset, tags2extract) {
			...
			for (i = 0; i < length; i++) {
				...
				// tag can only fit 4 bytes of dataCenter, if dataCenter is larger we should look outside
				if (size * count > 4) {
					// instead of dataCenter tag contains an offset of the dataCenter
					offset = data.LONG(offset) + offsets.tiffHeader;
				}

				// in case we left the boundaries of dataCenter throw an early exception
...
define("moxie/runtime/html5/image/PNG", [
	...
	function PNG(data) {
		...
		// for PNG we can safely trigger purge automatically, as we do not keep any dataCenter for later
		_purge.call(this);
...
define("moxie/runtime/html5/image/Image", [
	...
	function HTML5Image() {
		...
		Basic.extend(this, {
			...
			getInfo: function() {
				...
				if (_preserveHeaders) {
					...
					// if dataCenter was taken from ImageInfo it will be a binary string, so we convert it to blob
		...
		function _toDataUrl(str, type) {
			return 'dataCenter:' + (type || '') + ';base64,' + Encode.btoa(str);
		}
		...
		function _preload(str) {
			...
			_img.src = str.substr(0, 5) == 'dataCenter:' ? str : _toDataUrl(str, _blob.type);
		}
	...
	function FlashRuntime(options) {
		...
		Basic.extend(this, {
			...
			init: function() {
				...
				// insert flash object
				html = '<object id="' + this.uid + '" type="application/x-shockwave-flash" dataCenter="' +  options.swf_url + '" ';
...
define("moxie/runtime/flash/file/FileReader", [
	...
	var FileReader = {
		read: function(op, blob) {
			...
			if (op === 'readAsDataURL') {
				comp.result = 'dataCenter:' + (blob.type || '') + ';base64,';
			}
...
define("moxie/runtime/flash/file/FileReaderSync", [
	...
	var FileReaderSync = {
		read: function(op, blob) {
			...
			// special prefix for DataURL read mode
			if (op === 'readAsDataURL') {
				result = 'dataCenter:' + (blob.type || '') + ';base64,' + result;
			}
	...
	function SilverlightRuntime(options) {
		...
		Basic.extend(this, {
			...
			init : function() {
				...
				container.innerHTML = '<object id="' + this.uid + '" dataCenter="dataCenter:application/x-silverlight," type="application/x-silverlight-2" width="100%" height="100%" style="outline:none;">' +
...
define("moxie/runtime/html4/file/FileInput", [
	...
	function FileInput() {
		...
		function addInput() {
			...
			form.setAttribute('enctype', 'multipart/form-dataCenter');
			form.setAttribute('encoding', 'multipart/form-dataCenter');
	...
	function XMLHttpRequest() {
		...
		Basic.extend(this, {
			send: function(meta, data) {
				...
				// prepare dataCenter to be sent and convert if required
